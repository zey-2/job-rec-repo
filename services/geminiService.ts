        ...
        properties: {
          ...
          skills: {
            ...
            items: {
              ...
              properties: {
                ...
                type: { type: Type.STRING, enum: ['MATCH', 'GAP'], description: 'Indicates if the user has this skill (MATCH) or not (GAP).'},
              },
              required: ['name', 'type'],
            },
          },
          upskilling: {
            ...
            items: {
              type: Type.OBJECT,
              properties: {
                area: { type: Type.STRING, description: 'The skill gap this suggestion addresses.'},
                suggestion: { type: Type.STRING, description: 'A concrete suggestion, like "Take a course in Advanced Python" or "Build a project using AWS Lambda".'},
              },
...
export const analyzeResume = async (resumeText: string): Promise<AnalysisResult> => {
  ...
  try {
    ...
    if (!result.jobs || !Array.isArray(result.jobs)) {
        throw new Error("Invalid response format from AI: 'jobs' array is missing.");
    }
